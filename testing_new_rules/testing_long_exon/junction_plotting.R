...
old_annotations <- read_tsv('/project2/yangili1/qhauck/nmd_splicing_rules/testing_new_rules/testing_coordinate_system/Leaf2_junction_classifications.txt') %>%
...
# old_annotations <- read_tsv('/project2/yangili1/qhauck/nmd_splicing_rules/testing_new_rules/testing_long_exon/long_exon_passing_classifications.txt') %>%
#   separate(Intron_coord, into=c("chrom", "start", "end"), sep="[:-]", convert=T, remove = F) %>%
#   add_count(Intron_coord, name="NumberOfEntriesWithSameCoords")

old_annotations.filtered <- old_annotations %>%
...
juncs.long.summary.joined <- juncs.long.summary %>%
  ...
  mutate(stop = stop+1) %>%
  inner_join(old_annotations.filtered, by=c("chrom", "start", "stop"="end"))
...
old_coding_juncs <- filter(to_plot, Coding == TRUE)$Intron_coord
old_noncoding_juncs <- filter(to_plot, Coding == FALSE)$Intron_coord


new_annotations <- read_tsv('/project2/yangili1/qhauck/nmd_splicing_rules/testing_new_rules/testing_long_exon/Leaf2_junction_classifications.txt') %>%
  separate(Intron_coord, into=c("chrom", "start", "end"), sep="[:-]", convert=T, remove = F) %>%
  add_count(Intron_coord, name="NumberOfEntriesWithSameCoords")
