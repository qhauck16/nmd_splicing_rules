...
ggplot(to_plot, aes(x=Expression.Splicing, y=chRNA.Expression.Splicing)) +
  ...
  facet_grid(Coding~Long_exon, labeller = labeller(Long_exon = c(`TRUE` = "PTC only in Long Exon", `FALSE` = "PTC Elsewhere"),
...
ggplot(to_plot, aes(x=Expression.Splicing, y=chRNA.Expression.Splicing)) +
  ...
  scale_x_continuous(trans="log10", breaks=c(1E-1, 10, 1000), labels=c("0.1", "10", "1000")) +
  scale_y_continuous(trans="log10", breaks=c(1E-1, 10, 1000), labels=c("0.1", "10", "1000")) +
  facet_grid(Coding~Exons_before, labeller = labeller(Long_exon = c(`TRUE` = "4+ exons before PTC", `FALSE` = "Less than 4 exons before PTC"),
                                                   Coding = c(`TRUE` = "Productive", `FALSE` = "Unproductive"))) +
  theme_bw() +
  coord_fixed() +
  labs(x="Junction RPM, steady-state", y="Junction RPM, naRNA", title='Non-Coding vs Coding Splice Junctions w/long-exon rule')
...
ggplot(to_plot, aes(x=Expression.Splicing, y=chRNA.Expression.Splicing)) +
  ...
  scale_x_continuous(trans="log10", breaks=c(1E-1, 10, 1000), labels=c("0.1", "10", "1000")) +
  scale_y_continuous(trans="log10", breaks=c(1E-1, 10, 1000), labels=c("0.1", "10", "1000")) +
  facet_grid(Coding~Exons_after, labeller = labeller(Long_exon = c(`TRUE` = "4+ exons after PTC", `FALSE` = "Less than 4 exons after PTC"),
                                                      Coding = c(`TRUE` = "Productive", `FALSE` = "Unproductive"))) +
  theme_bw() +
  coord_fixed() +
  labs(x="Junction RPM, steady-state", y="Junction RPM, naRNA", title='Non-Coding vs Coding Splice Junctions w/long-exon rule')
